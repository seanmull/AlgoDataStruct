...
public class PlagiarismDetector {
	...
	public static Map<String, Integer> detectPlagiarism(String dirName, int windowSize, int threshold) {
		...
		ArrayList <HashSet<String>> filePhases = new ArrayList <HashSet<String>>();
		String checkedFile;
		String currentFile;
		String file;
		...
		for (int i = 0; i < files.length; i++) {
			file = files[i];
			HashSet<String> tempSet = (HashSet<String>) createPhrases(dirName + "/" + file, windowSize);
			filePhases.add(tempSet); 
		}
		
		
		//TODO load in all files
		...
		for (int i = 0; i < files.length; i++) {
			...
			for (int j = 0; j < files.length; j++) { 
				...
				currentFile = i + "_" + j;
				if(i == j || checkedFiles.contains(currentFile)){
					continue;
				}
				checkedFile = j + "_" + i;
				...
				//TODO pass in the right arrays
				Set<String> matches = findMatches(filePhases.get(i), filePhases.get(j));
	...
	//TODO optimize this as follows
	//Read in the words as a queue
	//read in every window of words the pop the top of the list
	protected static Set<String> readInPhrases(String filename, int window){
		if (filename == null) return null;
		...
		try {
			...
			while (in.hasNext()) {
				...
				if(counter % (window) == 0) {
					if(phrases.size() < 10) {
						//System.out.println(phrase.replaceAll("[^a-zA-Z\\s]", "").toUpperCase());
					}
					phrases.add(phrase.replaceAll("[^a-zA-Z\\s]", "").toUpperCase());
					phrase = "";
				}
			}
		}
